...
- _Resource Code_: In the resource code (e.g., `external/service/{service}/{thing}.go`),
...
- _Resource Acceptance Tests_: In the resource acceptance tests (e.g., `external/service/{service}/{thing}_test.go`), implement one or more tests containing a `TestStep` with `ImportState: true`.
