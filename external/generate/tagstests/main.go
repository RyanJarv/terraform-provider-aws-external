...
import (
	...
	acctestgen "github.com/hashicorp/terraform-provider-aws/external/acctest/generate"
	"github.com/hashicorp/terraform-provider-aws/external/generate/common"
	tfmaps "github.com/hashicorp/terraform-provider-aws/external/maps"
...
func main() {
	...
	g.Infof("Generating tagging tests for external/service/%s", servicePackage)
	...
	for _, l := range serviceData {
		// See external/generate/namesconsts/main.go.
				...
				if attr, ok := args.Keyword["checkDestroyNoop"]; ok {
					if b, err := strconv.ParseBool(attr); err != nil {
						...
						d.GoImports = append(d.GoImports,
							goImport{
								Path: "github.com/hashicorp/terraform-provider-aws/external/acctest",
							},
						)
					}
				}
